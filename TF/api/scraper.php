<?php
/**
 * Athletic Records Scraper
 * Fetches data from alltime-athletics.com and caches it as JSON
 * 
 * Usage:
 *   - api/scraper.php?event=m_mara     (fetch men's marathon)
 *   - api/scraper.php?event=m_mara&refresh=1  (force refresh cache)
...
// Note: url_code must match the actual filename on alltime-athletics.com
$EVENTS = [
    'mmara'   => ["Men's Marathon", 'mmara', 600000],
    'mhmara'  => ["Men's Half Marathon", 'mhmara', 280000],
    'm10000'  => ["Men's 10,000", 'm10000', 140000],
    'm_5000'  => ["Men's 5,000", 'm_5000', 82000],
    'm_3000'  => ["Men's 3,000", 'm_3000', 47000],
    'w_5000'  => ["Women's 5,000", 'w_5000', 150000],
    'w_3000'  => ["Women's 3,000", 'w_3000', 199000],
];
...
if (isset($_GET['event'])) {
    ...
    if (!isset($EVENTS[$event])) {
        echo json_encode(['error' => 'Unknown event. Use ?list to see available events.']);
        exit;
    }
...
// Default: show help
echo json_encode([
    'name' => 'Athletics Records API',
    'endpoints' => [
        '?list' => 'List all available events',
        '?event=EVENT_CODE' => 'Get data for specific event (e.g., ?event=m_mara)',
        '?event=EVENT_CODE&refresh=1' => 'Force refresh cache for event',
        '?update_all=1' => 'Update all events (for cron job)'
    ],
    'example_events' => ['m_mara', 'm_hmara', 'm_10k', 'm_5000', 'm_3000', 'w_5000', 'w_3000']
], JSON_PRETTY_PRINT);
